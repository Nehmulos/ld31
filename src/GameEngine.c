...
GameEngine* GameEngine_create(void) {
    ...
    cpSpaceSetGravity(this->space, cpv(0, 0.0001));
    ...
    for (int i=0; i < PLAYER_COUNT; ++i) {
        ...
        Input_loadDefault_1(this->input[i]);
...
void GameEngine_guileInit(void *data) {
    scm_c_eval_string("(add-to-load-path (getcwd))");
}
...
void GameEngine_initSubsystems() {
    ...
    GuileTypes_registerAll();
    // init load path
    scm_with_guile(GameEngine_guileInit, NULL);
...
void GameEngine_debugDraw(GameEngine* this, struct Camera* camera) {
    struct ChipmunkDebugDrawData data = {
        .renderer = this->renderer,
        .camera = camera
    };
    cpSpaceDebugDrawOptions debugDrawOptions = ChipmunkDebugDrawSDL2_make(&data);

    cpSpaceDebugDraw(this->space, &debugDrawOptions);
...
int GameEngine_run(GameEngine* this) {
    ...
    while(this->windowIsRunning) {
        ...
        while (now - lastUpdate > UPDATE_TIME) {
            cpSpaceStep(this->space, UPDATE_TIME);
